...
const aggregatePackageByDaySend = (pacs: any[]): { [key: string]: number } =>
    pacs
        .reduce((acc: any, curr: any) => {
            const day = format(new Date(curr.send_date), 'yyyy-MM-dd');
            if (!acc[day]) {
                acc[day] = 0;
            }
            acc[day] += 1;
...
const aggregatePackageByDayRecv = (pacs: any[]): { [key: string]: number } =>
    pacs
        .reduce((acc: any, curr: any) => {
            const day = format(new Date(curr.receive_date), 'yyyy-MM-dd');
            if (!acc[day]) {
                acc[day] = 0;
            }
            acc[day] += 1;
...
const aggregatePackageByMonthSend = (pacs: any[]): { [key: string]: number } =>
    pacs
        .reduce((acc: any, curr: any) => {
            const day = format(new Date(curr.send_date), 'yyyy-MM');
            if (!acc[day]) {
                acc[day] = 0;
            }
            acc[day] += 1;
...
const aggregatePackageByMonthRecv = (pacs: any[]): { [key: string]: number } =>
    pacs
        .reduce((acc: any, curr: any) => {
            const day = format(new Date(curr.receive_date), 'yyyy-MM');
            if (!acc[day]) {
                acc[day] = 0;
            }
            acc[day] += 1;
...
const PackageStatistic = (smallestYear: any) => {
    ...
    const [filter, setFilter] = React.useState({
        send_date_gt: (new Date("01/01/" + year.toString())).toISOString(),
        send_date_lte: (new Date("01/01/" + (year + 1).toString())).toISOString(),
    });
    const [filter1, setFilter1] = React.useState({
        final_state: 'finished',
        receive_date_gt: (new Date("01/01/" + year.toString())).toISOString(),
        receive_date_lte: (new Date("01/01/" + (year + 1).toString())).toISOString(),
    });
    const [filter2, setFilter2] = React.useState({
        final_state: 'not-finished',
        receive_date_gt: (new Date("01/01/" + year.toString())).toISOString(),
        receive_date_lte: (new Date("01/01/" + (year + 1).toString())).toISOString(),
    });
    ...
    const handleMonthChange = (event: SelectChangeEvent) => {
        ...
        if (parseInt(event.target.value) !== 0) {
            if (parseInt(event.target.value) !== 12) {
                setFilter({
                    send_date_gt: (new Date(`${event.target.value}/01/` + year.toString())).toISOString(),
                    send_date_lte: (new Date(`${(parseInt(event.target.value) + 1).toString()}/01/` + (year).toString())).toISOString(),
                });
            }
            else {
                setFilter({
        ...
        if (parseInt(event.target.value) !== 0) {
            if (parseInt(event.target.value) !== 12) {
                setFilter1({
                    final_state: 'finished',
                    receive_date_gt: (new Date(`${event.target.value}/01/` + year.toString())).toISOString(),
                    receive_date_lte: (new Date(`${(parseInt(event.target.value) + 1).toString()}/01/` + (year).toString())).toISOString(),
                });
            }
            else {
                setFilter1({
    ...
    const handleYearChange = (event: SelectChangeEvent) => {
        ...
        if (month !== 0) {
            if (parseInt(event.target.value) !== 12) {
                setFilter({
                    send_date_gt: (new Date(`${month.toString()}/01/` + year.toString())).toISOString(),
                    send_date_lte: (new Date(`${(month + 1).toString()}/01/` + (year).toString())).toISOString(),
                });
            }
            else {
                setFilter({
        ...
        if (month !== 0) {
            if (parseInt(event.target.value) !== 12) {
                setFilter1({
                    final_state: 'finished',
                    receive_date_gt: (new Date(`${month.toString()}/01/` + year.toString())).toISOString(),
                    receive_date_lte: (new Date(`${(month + 1).toString()}/01/` + (year).toString())).toISOString(),
                });
            }
            else {
                setFilter1({
    ...
    const { data: curPackageSend, isLoading: isLoading } = useGetList('exchangingPackage', { filter: filter });
    const { data: curPackageReceive, isLoading: isLoading1 } = useGetList('exchangingPackage', { filter: filter1 });

    const { data: curPackageFail, isLoading: isLoading2 } = useGetList('exchangingPackage', { filter: filter2 });
    const { data: curPackageSending, isLoading: isLoading3 } = useGetList('exchangingPackage', { filter: filter3 });
                ...
                <CardContent>
                    <Stack direction="row" spacing={2}>
                        ...
                        <Select
                            ...
                            disabled={isLoading || isLoading1 || isLoading2 || isLoading3}
                        >
                    ...
                    {isLoading || isLoading1 || isLoading2 || isLoading3 ? (
                        ...
                        (
                            <>
                                <Stack direction="row" spacing={2}>
                                    <SendCard value={curPackageSend?.length} />
                                    <ReceiveCard value={curPackageReceive?.length} />
                                    <SendingCard value={curPackageSending?.length} />
                                    <FailCard value={curPackageFail?.length} />
                                </Stack>
                                <Typography variant="h6" gutterBottom>
                                    {`Thống kê hàng gửi ${month !== 0 ? month.toString() + '/' + year.toString() : year.toString()}`}
                                </Typography>
                                ...
                                <Typography variant="h6" gutterBottom>
                                    {`Thống kê hàng nhận ${month !== 0 ? month.toString() + '/' + year.toString() : year.toString()}`}
                                </Typography>
                                <div style={{ width: '100%', height: 300 }}>
                                    <ResponsiveContainer>
                                ...
                                <Typography variant="h6" gutterBottom>
                                    {`Thống kê hàng chuyển lỗi ${month !== 0 ? month.toString() + '/' + year.toString() : year.toString()}`}
                                </Typography>
                                <div style={{ width: '100%', height: 300 }}>
                                    <ResponsiveContainer>
                                        <AreaChart data={month !== 0 ? getTotalPerDayRecv(month, year, curPackageFail) : getTotalPerMonthRecv(year, curPackageFail)}>
                                            <defs>
                                                <linearGradient
                                                    id="colorUv"
                                                    x1="0"
                                            ...
                                            <Tooltip
                                                cursor={{ strokeDasharray: '3 3' }}
                                                formatter={(value: any) =>
                                                    value.toString() + " hàng chuyển lỗi"
                                                }
                                                labelFormatter={(label: any) =>
                                                    dateFormatter(label)
                                                }
